...
function composeSheet({ summary = '', sceneLine = '', timeline = '', need = [], recovered = [], settings = {}, databases = {}, runId = '' } = {}) {
    ...
    // "Story so far:" is append-only and was 34 k chars by turn 150 of the
    // measured run — the newest sentences are the ones the next beat leans on,
    // so the budget keeps the tail and says how many batches it skipped.
    if (spineText) {
        let spineSentences = [];
        try { spineSentences = getStorySpine().map(b => String(b.sentence || '').trim()).filter(Boolean); } catch { spineSentences = [spineText]; }
        const kept = [];
        let used = 0;
        for (let i = spineSentences.length - 1; i >= 0; i--) {
            const cost = spineSentences[i].length + 1;
            if (kept.length > 0 && used + cost > budget.story) break;
            kept.unshift(spineSentences[i]);
            used += cost;
        }
        const skipped = spineSentences.length - kept.length;
        if (skipped > 0) capped.story = { dropped: skipped, kept: kept.length, chars: used, budget: budget.story };
        lines.push(`Story so far: ${skipped > 0 ? `…(${skipped} earlier batches) ` : ''}${kept.join(' ')}`);
    }
